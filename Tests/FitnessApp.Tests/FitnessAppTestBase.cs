...
namespace FitnessApp.Tests
{
    public abstract class FitnessAppTestBase : AbpIntegratedTestBase<FitnessAppTestModule>
    {
        ...
        protected void UsingDbContext(Action<FitnessAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FitnessAppDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FitnessAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FitnessAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FitnessAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FitnessAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FitnessAppDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FitnessAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FitnessAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FitnessAppDbContext, Task<T>> func)
        {
            T result;
